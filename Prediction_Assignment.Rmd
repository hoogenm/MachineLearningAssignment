...
Note: the document has between _approximately_ 1,430 words (counted from Word-version using OpenOffice) and 1,987 words (counted from a PDF-version by [this online tool](http://felix-cat.com/tools/wordcount/)).
...
2. `caret` will already use resampling internally (therefore I think the risk of overfitting the model is already mitigated) 3. I want the model trained more thoroughly to use it on the official `testing` set (that I have set aside).
