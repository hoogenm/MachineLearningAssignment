...
75% training data (instead of a lower value of 60% e.g.) because the training set is not too small, because `caret` will already use resampling internally (therefore I think the risk of overfitting the model is already mitigated), and
because I want the model trained more thoroughly to use it on the official `testing` set that is set aside.

I will use the `set.seed` function to make my work reproducible.
...
## Training the model
Package `caret` provides a training function that can be used with Random Forest and also provides sensible defaults. The package also provides multicore/parallel processing (if a backend is registered), and because of the size of the
...
To write out the final results to (individual) answer-files, I used the script that is suggested by the course assignment:
...
The creation of the answer files ends this writeup (and provides the files necessary for the 'submission' part of the assignment).